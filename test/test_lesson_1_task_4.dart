  ...
  testWidgets('module4', (WidgetTester tester) async {
    ...
    final columnFinder = find.descendant(
      ...
      matching: find.descendant(
          of: find.byType(Stack), matching: find.byType(Column)),
    );
    ...
    final centerFinder =
        findTypeByChildTypeOnlyInParentType(Center, Column, Stack);
    final Iterable<Center> centerWidgets = tester.widgetList(centerFinder);
    if (centerWidgets.length == 0) {
      final alignFinder =
          findTypeByChildTypeOnlyInParentType(Align, Column, Stack);
      expect(
        alignFinder,
        findsOneWidget,
        reason:
    ...
    final stackInColumnFinder =
        find.descendant(of: columnFinder, matching: find.byType(Stack));
    ...
    expect(
      ...
      2,
      reason:
          "There should be 2 widgets inside Stack: one with circle and other with image",
    );
    ...
    checkContainerDecorationColor(
        container: container, color: const Color(0xFF00BCD4));
    ...
    final Text noFavoritesYetText =
        tester.firstWidget(noFavoritesYetTextFinder);
    ...
    expect(
      ...
      20,
      reason:
          "SizedBox between widget with blue circle with iron man and title should be 20",
    );
